    ...
    if !state.room_members.contains(&format!("{}", participant_pk)) {
        ...
        loop {
            match response_receiver.recv().await {
                ...
                Err(e) => {
                    println!("❌ Failed to receive join confirmation: {}", e);
                    println!("💡 This might be due to network issues or episode engine problems");
                    return;
                }
            }
        }
    ...
    if !state.authenticated_users.contains(&format!("{}", participant_pk)) {
        ...
        loop {
            match response_receiver.recv().await {
                ...
                Err(e) => {
                    println!("❌ Failed to receive challenge: {}", e);
                    println!("💡 This might be due to network issues or episode engine problems");
                    return;
                }
            }
        ...
        if let Some(challenge_text) = challenge {
            ...
            loop {
                match response_receiver.recv().await {
                    ...
                    Err(e) => {
                        println!("❌ Failed to receive authentication confirmation: {}", e);
                        println!("💡 This might be due to network issues or episode engine problems");
                        return;
                    }
                }
    ...
    loop {
        ...
        loop {
            match response_receiver.recv().await {
                ...
                Err(e) => {
                    println!("❌ Failed to receive response from episode engine: {}", e);
                    println!("💡 This might be due to network issues or episode engine problems");
                    break;
                }
            }
